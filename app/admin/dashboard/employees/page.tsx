...
export default function EmployeesPage() {
  ...
  useEffect(() => {
    const fetchEmployees = async () => {
      const { data } = await supabase
        ...
        .neq('role', 'admin')
      setEmployees((data || []).map(({ id, email }) => ({ id, email })))
      setLoading(false)
    }
    fetchEmployees()
  }, [])
  ...
  return (
    <div className='w-full max-w-4xl mx-auto p-8 rounded-2xl shadow-xl bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800'>
      <h1 className='text-2xl font-bold text-blue-900 dark:text-blue-300 mb-6'>
        Employees
      </h1>
          ...
          <tbody>
            {loading ? (
              ...
              employees.map((emp) => (
                <tr
                  key={emp.id}
                  className='hover:bg-blue-50 dark:hover:bg-neutral-800 transition'
                >
